		...
		if(started){
			...
			if(coord.form == 0){//Line Formation
				...
				if(change_pauser){//Slow down for diamond
					...
					if(!change_pauser && count < 100){//Ensures back can catch up
						ppp[0] -> GoTo(coord.x - .5, coord.y + 0.967, 0);
						ppp[1] -> GoTo(coord.x - .5, coord.y - 1.035, 0);
						ppp[2] -> GoTo(coord.x, coord.y, 0);
						count++;
					}
					else{
